...
Меню состоит из одного элемента - `<ul>`, и его детей - `<li>`. Внутри каждого элемента списка есть два `<span>` - название блюда и цена. На каждом `<li>` висит `display: flex` и `justify-content: space-between`, благодаря которым название и цена блюда расположены по разным сторонам.
